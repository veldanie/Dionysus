...
cerr << "LOGGING" << endl;
  ...
  process_command_line_options(argc, argv, 
			skeleton_dimension, multiplier, infilename, outfilename);


  // Read in points
  PointContainer points;
  read_points(infilename, points);
cerr << "read points" << endl;
...
cerr << "computed distances" << endl;
// total.check ("Total timer ");


	typedef Rips<PairDistances> Generator;
	// Generator rips(distances);
	Rips<PairDistances> rips(distances);
cerr << "constructed Rips<Distances>" << endl;
// total.check ("Total timer ");

	Rips<PairDistances>::Evaluator size(distances);
	typedef Generator::Simplex Smplx;
	typedef Filtration<Smplx> Fltr;
	Fltr f;


cerr << "generating rips" << endl;
	rips.generate(2, 10, make_push_back_functor(f));
cerr << "generated rips, size = " << f.size() << endl;
// total.check ("Total timer ");
cerr << "------------------------------------" << endl;
cerr << f;
cerr << "------------------------------------" << endl;
...
cerr << "recorded persistence intervals in the persistence diagrams" << endl;
// total.check ("Total timer ");
...
// Serialize the diagrams to a file
	...
	oa << dgms;
	cerr << "output rips-diagrams" << endl;
// total.check ("Total timer ");
...
cerr << "Point and epsilon ordering:" << endl;
for (unsigned i = 0; i < vertices.size(); ++i)
	fprintf(stderr, "  %4d: %4d - %f\n", i, vertices[i], epsilons[i]);
...
cerr << "Total participating edges: " << edges.size() << endl;
for (EdgeVector::const_iterator cur = edges.begin(); cur != edges.end(); ++cur)
fprintf(stderr, "  (%d, %d) %f\n", cur->first, cur->second, distances(cur->first, cur->second));
...
fprintf(stderr, "Adding %s", tostring(sv).c_str());
...
cerr << "Commencing computation" << endl;
...
fprintf(stderr, "Current stage %d: %d %f: %f", stage,
 vertices[i], epsilons[i-1],
 multiplier*epsilons[i-1]);
...
cerr << "  Cofaces size: " << cofaces.size() << endl;
