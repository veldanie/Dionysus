 ...
 * Rips class
 *
 * Class providing basic operations to work with Rips complexes. 
 * It implements Bron-Kerbosch algorithm, 
...
template<
		class Distances_, 
		class Simplex_ = Simplex<typename Distances_::IndexType>
	>
class Rips
{
public:
...
// Calls functor f on each simplex in the k-skeleton of the Rips complex
	...
	void generate(
		...
		DistanceType max,
		const Functor& f, 
		Iterator candidates_begin,
		Iterator candidates_end
	) const;
  ...
  /* No Iterator argument means Iterator = IndexType and the range is [distances().begin(), distances().end()) */
	...
	) const
	{
		generate(k,
				max,
				f,
				boost::make_counting_iterator(distances().begin()),
				boost::make_counting_iterator(distances().end()));
	}
